...
def main():
    ...
    with left:
        lang_mode = st.selectbox("Choose your language",
                                 ["en-US", "zh-CN", "fr-FR", "ja-JP", "it-IT", "pt-PT", "ru-RU"], key='lang')
    with right:
        int_mode = st.selectbox('Choose the model', ["high Intelligence", "medium Intelligence", "low Intelligence"],
                                key='intel')
    ...
    with st.sidebar:
        ...
        stun_mode = st.selectbox("Choose the stun server", ["google1", "中国", "google2", "google3", "google4"],
                                 key='stun')
        if stun_mode == '中国':
...
def app_sst_side():
    ...
    sugg = suggestion_ai(conversation_sugg, sugg_mod)
